...
git rm --cached ppob_visibility.json 2>nul
del /f /q ppob_visibility.json 2>nul
echo.
echo [2/4] Menyiapkan perubahan (git add)...
git add .
echo.
echo [3/4] Menyimpan commit (git commit)...
git commit -m "fix: remove ppob_visibility.json from git to prevent merge conflict"
echo.
